...
import { IColumn, IGroup, SavedTableView, TableView } from "../types";
...
>(props: TableProps<TItem>, providedColumns: IColumn<TItem>[]) {
  ...
  const { filter, setFilterValue, resetFilterValue, applyFilter } =
    useTableFilter();
  ...
  const {
    ...
    visibleColumns,
    setVisibleColumns,
  } = useTableColumns<TItem>(providedColumns);
  ...
  const pagedItems = React.useMemo(() => {
    ...
    if (isPageOnGroup && groupedColumns.length > 0) {
      ...
      startIndex =
        pGroups?.length > 0 ? pGroups?.[currentPage * pageSize]?.startIndex : 0;
      ...
      pGroups = calculateGroups(groupedColumns, pItems, providedColumns);
  ...
  const saveTableView = React.useCallback(
    (viewName: string) => {
      //todo
      ...
      const newSetOfViews: SavedTableView = {
        views: [
          ...(views?.filter((v) => v.viewName !== viewName) ?? []),
          newTableView,
        ],
      };
  ...
  const applyTableView = React.useCallback(
    (viewName: string) => {
  ...
  const showNoItem = React.useMemo(
    () => !isLoading && items?.length === 0,
    [isLoading, items]
  );
  const showNoItemMatch = React.useMemo(
    () => pagedItems?.length === 0 && items?.length > 0,
    [isLoading, pagedItems, items]
  );
